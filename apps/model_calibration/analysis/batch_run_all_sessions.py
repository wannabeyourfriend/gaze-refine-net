...
def read_samples_target_files(folder_path):
    """Read all samples_target_* files from folder into dict."""
    samples_dict = {}
...
def create_samples_columns(test_df, test_samples_dict):
    """Add test samples data columns to dataframe."""
    result_columns = {}
    n_rows = len(test_df)
...
def process_one_trial(subject, timestamp, origin_path, test_path):
    ...
    from model_calibration.models.calibration_model_full_compare import RBF_PARAM_GRID
    ...
    # Add test samples data columns to output
