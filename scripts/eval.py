...
def parse_csv_file(filepath: str, method: str) -> Tuple[List[float], int, int]:
    """
    Parse CSV file and calculate error distances.
    
    Args:
        filepath: Path to the CSV file
        method: Method to evaluate ("original", "sim_rbf", or "pred_gaze")
    
    Returns:
        Tuple of (list of distances, valid_count, total_count)
    """
    distances = []
    total_count = 0
    valid_count = 0
    ...
    if method == "original":
        gaze_x_col = "original_gaze_x"
        gaze_y_col = "original_gaze_y"
    elif method == "sim_rbf":
        gaze_x_col = "sim_rbf_gaze_x"
        gaze_y_col = "sim_rbf_gaze_y"
    elif method == "pred_gaze":
        gaze_x_col = "pred_gaze_x"
        gaze_y_col = "pred_gaze_y"
    else:
...
def main():
    ...
    # Default data file name if not provided
