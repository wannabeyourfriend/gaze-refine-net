...
gaze-refine-net is a Python eye-tracking calibration system that combines traditional model-based approaches (polynomial fitting, RBF) with deep learning refinement. Designed for Pupil Labs eye-tracking hardware.
...
2. **Preprocessing**: Raw data → `scripts/process_data.py` → train/val/test.csv
...
| `apps/user_identify/` | Subject identification from gaze sequences |
| `apps/figuring/` | Gaze visualisation figures |
| `scripts/` | Pipeline and analysis scripts (see `scripts/README.md`) |
| `checkpoints/` | Empty on clone; training writes here |
